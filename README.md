...
- **`generateCandidateSpecials`**  
  Generates candidate specials from a bar website. It accepts `homepage_url`, `bar_name`, and `neighborhood`, crawls same-domain links that look like specials/happy-hour/menu/event pages, extracts text, and sends that text to OpenAI for strict JSON extraction. If crawl-based extraction returns no specials, it falls back to an OpenAI `web_search` flow. Output is an array of normalized specials with `description`, `type`, `days_of_week`, `start_time`, `end_time`, `all_day`, `confidence`, `notes`, and `source`.
