...
- **`googleBarSync`**  
  This is the only entry point for the new two-Lambda bar sync flow. It accepts only `{ "neighborhood": "downtown" }`, loads the built-in neighborhood config, searches Google Places, filters candidates to the configured polygon, formats open hours using the same structure as `fetchGoogleAPIHours`, asks `dbBarSync` which bars are new vs existing, fetches Google photos only for new bars, uploads those images to S3, and then calls `dbBarSync` again to save the results.
