...
- Confidence scoring rule: set confidence to 1.0 ONLY when the item has a clear deal + clear time window AND includes a deal signal (special/deal/promo/happy hour) or a dollar sign ($).
...
def _contains_deal_signal(text):
    blob = (text or '').lower()
    if '$' in blob:
        return True
    if re.search(r'\b\d+\s*[%]?\s*off\b', blob):
        return True
    return any(term in blob for term in SPECIALS_VOCAB)
...
def _apply_crawl_quality_rules(items):
    ...
    for item in items:
        ...
        has_deal = _contains_deal_signal(blob)
        if has_time and has_deal:
            item['confidence'] = 1.0
